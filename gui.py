...
def gui():
	...
	id_list=[]
	for i in range(0, 5):
		#print user[int(top_5[i])].age,user[int(top_5[i])].sex,user[int(top_5[i])].occupation,user[int(top_5[i])].id
		if user[int(top_5[i])].id<=671:
			id_list.append(str(user[int(top_5[i])].id))
		maxi = 0
		maxe = 0
		for j in range(0, 19):
